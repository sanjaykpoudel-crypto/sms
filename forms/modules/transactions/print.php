...
// Fetch Header
$header = $db->fetchOne("
    SELECT t.*, u.full_name as created_by_name, loc.name as location_name 
...
if ($txn_type == 'journal_entry' || $txn_type == 'Journal') {
    $items = $db->fetchAll("
        SELECT je.*, a.account_name as item_name, je.entry_type
    ...
    $items = $db->fetchAll("
        SELECT tl.*, i.item_name, i.sku, loc.name as line_location_name
...
<head>
    <meta charset="UTF-8">
    <title><?php echo htmlspecialchars($displayType); ?> - <?php echo htmlspecialchars($header['txn_number']); ?></title>
    <style>
        @media print {
        ...
        .print-btn:hover { background: #002266; }
    </style>
</head>
<body>
    ...
    ?>
    <div class="header" style="display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid #0f172a; padding-bottom: 12px; margin-bottom: 15px;">
        <div class="header-logo" style="flex: 0 0 140px; text-align: left;">
            <?php if (!empty($sys_logo) && file_exists($sys_logo)): ?>
                <img src="<?php echo htmlspecialchars($sys_logo); ?>" alt="Logo" style="max-height: 75px; max-width: 130px; object-fit: contain;">
            <?php else: ?>
                <div style="font-size: 26px; font-weight: 900; color: #003087; line-height: 1; letter-spacing: -0.5px;"><?php echo htmlspecialchars($sys['short_name'] ?? 'MNS'); ?></div>
            <?php endif; ?>
        </div>
        <div class="header-details" style="flex: 1; text-align: right;">
            <div class="company-name" style="font-size: 22px; font-weight: 800; color: #003087; margin-bottom: 2px; text-transform: uppercase;"><?php echo htmlspecialchars($sys_company_name); ?></div>
            <div class="company-info" style="font-size: 12px; color: #475569; line-height: 1.3;">
                ...
                <?php if(!empty($sys_company_email)): ?> | Email: <?php echo htmlspecialchars($sys_company_email); ?><?php endif; ?>
                <br>
        ...
        <div class="info-box">
            <table>
                <tr>
                    ...
                    <td><strong><?php echo htmlspecialchars($details['entity_name'] ?? 'General Party'); ?></strong></td>
                </tr>
        ...
        <div class="info-box">
            <table>
                <tr>
                    ...
                    <td><strong style="color: #003087; font-size: 14px;"><?php echo htmlspecialchars($header['txn_number']); ?></strong></td>
                </tr>
                <tr>
                    <td>Date:</td>
                ...
                <tr>
                    <td>Location:</td>
                    <td><strong><?php echo htmlspecialchars($header['location_name'] ?? 'Gokarna'); ?></strong></td>
                </tr>
                <tr>
                    <td>Status:</td>
                    <td><span style="font-weight: bold; color: <?php echo $printStatusColor; ?>;"><?php echo htmlspecialchars($printStatusStr); ?></span></td>
                </tr>
                <tr>
                    <td>Prepared By:</td>
    ...
    <table class="items-table">
        <thead>
            ...
            <tr>
                <th width="6%">S.N.</th>
                <th width="<?php echo $txn_type == 'inventory_adjustment' ? '30%' : '44%'; ?>">Description of Goods / Services</th>
        ...
        <tbody>
            ...
            foreach($items as $item): 
                if ($txn_type == 'journal_entry' || $txn_type == 'Journal') {
                    ...
                    if ($is_debit) $tot_debit += $amt; else $tot_credit += $amt;
            ?>
            <tr>
            ...
            <tr>
                ...
                <td style="text-align:center;">
                    <span style="background:#e0f2fe;color:#0369a1;padding:1px 6px;border-radius:8px;font-size:10px;font-weight:700;">
                        <?php echo htmlspecialchars($item['line_location_name'] ?? $header['location_name'] ?? '—'); ?>
                    </span>
                </td>
                <?php endif; ?>
                ...
                <td class="text-right" style="font-weight: 600;"><?php echo number_format($item['line_total'] ?? 0, 2); ?></td>
            </tr>
            <?php 
                }
            endforeach; 
    ...
    <div class="totals-container">
        <table class="totals-table">
            ...
            <tr style="font-size: 15px;">
                ...
                <td class="text-right" style="color: #059669;"><strong>Rs <?php echo number_format($tot_debit, 2); ?></strong></td>
            </tr>
            <?php else: ?>
            <tr>
                <td>Sub Total:</td>
            ...
            <tr style="font-size: 15px;">
                ...
                <td class="text-right" style="color: #003087; font-size: 16px;"><strong>Rs <?php echo number_format($grand_total, 2); ?></strong></td>
            </tr>
            <?php endif; ?>
        </table>
    </div>
    ...
    ?>
    <!-- PAYMENT QR CODE BOX -->
    <div style="display: flex; align-items: center; justify-content: space-between; border: 1.5px dashed <?php echo $is_unpaid ? '#e74c3c' : '#003087'; ?>; border-radius: 8px; padding: 10px 16px; margin: 12px 0; background: <?php echo $is_unpaid ? '#fff5f5' : '#f8fafc'; ?>;">
        <div>
            <div style="font-size: 13px; font-weight: 800; color: <?php echo $is_unpaid ? '#c0392b' : '#003087'; ?>; text-transform: uppercase; letter-spacing: 0.5px;">
                <i class="fas fa-qrcode"></i> <?php echo htmlspecialchars($qr_title); ?>
            </div>
            <div style="font-size: 11px; color: #475569; margin-top: 4px;">
                Invoice No: <strong><?php echo htmlspecialchars($header['txn_number']); ?></strong> | <?php echo $is_unpaid ? 'Unpaid Balance' : 'Amount Due'; ?>: <strong style="color: #c0392b; font-size:12px;">Rs <?php echo number_format($unpaid_amount, 2); ?></strong>
            </div>
        ...
        <div style="text-align: center;">
            <img src="<?php echo htmlspecialchars($qr_src); ?>" alt="Payment QR" style="width: 85px; height: 85px; border-radius: 4px; border: 1px solid #cbd5e1; background: #fff; padding: 2px;">
        </div>
    </div>
    <?php endif; ?>
