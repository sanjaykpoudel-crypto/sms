...
<script>
    ...
    function vcFetchItem(select) {
        ...
        fetch('api/get_item_details.php?id=' + itemId)
            ...
            .then(data => {
                ...
                if (rateInput) rateInput.value = parseFloat(data.cost_price || 0).toFixed(2);
                ...
                if (conv > 1) {
                    unitTd.innerHTML = `
                        <select name="unit[]" class="ns-select unit-input" style="padding: 2px 4px; font-size: 11px; font-weight: 700; color: #0369a1; background: #e0f2fe; border: 1px solid #7dd3fc; border-radius: 4px;" onchange="vcUnitChanged(this)">
                ...
                updateVcStockDisplay(tr);
                vcCalcRow(tr.querySelector('.qty-input'));
            });
    }

    function vcUnitChanged(select) {
        const tr = select.closest('tr');
    ...
    function updateVcStockDisplay(tr) {
