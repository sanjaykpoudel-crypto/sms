...
$sql = "
    SELECT 
        ...
        COALESCE(SUM(ci.balance_due), 0.00) as total_due,
        COALESCE(SUM(CASE WHEN DATEDIFF(?, ci.invoice_date) BETWEEN 0 AND 30 THEN ci.balance_due ELSE 0.00 END), 0.00) as bucket_30,
        COALESCE(SUM(CASE WHEN DATEDIFF(?, ci.invoice_date) BETWEEN 31 AND 60 THEN ci.balance_due ELSE 0.00 END), 0.00) as bucket_60,
        COALESCE(SUM(CASE WHEN DATEDIFF(?, ci.invoice_date) BETWEEN 61 AND 90 THEN ci.balance_due ELSE 0.00 END), 0.00) as bucket_90,
        COALESCE(SUM(CASE WHEN DATEDIFF(?, ci.invoice_date) > 90 THEN ci.balance_due ELSE 0.00 END), 0.00) as bucket_over_90
    FROM customers c
    ...
    <table class="ns-table" id="tbl-ar-aging">
      ...
      <tbody>
        <?php foreach ($rows as $r): ?>
          <tr>
            <td style="font-weight:700; color:#888;"><?= $r['customer_code'] ?></td>
