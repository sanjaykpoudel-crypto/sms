...
if ($raw_account === 'bank' || $raw_account === 'all_bank') {
  ...
  $cash_rows = $db->fetchAll("SELECT id FROM accounts WHERE account_subtype = 'Bank' AND is_active=1 AND is_deleted=0");
...
$accounts_list = $db->fetchAll("SELECT id, REPLACE(id, 'acc-', '') as account_code, account_name FROM accounts WHERE is_active=1 AND is_deleted=0 ORDER BY account_name ASC");
...
<div class="rpt-summary">
  <?php if (!empty($account_ids)): ?>
    <div class="rpt-summary-card">
      <div class="val"><?= rpt_currency($opening_bal) ?></div>
      <div class="lbl">Opening Balance</div>
    </div>
    <div class="rpt-summary-card">
      <div class="val" style="color:#003087"><?= rpt_currency($total_debit) ?></div>
      <div class="lbl">Total Debits (Dr)</div>
    </div>
    <div class="rpt-summary-card">
      <div class="val" style="color:#c00"><?= rpt_currency($total_credit) ?></div>
      <div class="lbl">Total Credits (Cr)</div>
    </div>
    <div class="rpt-summary-card">
      <div class="val" style="color:#1a7f37"><?= rpt_currency($closing_bal) ?></div>
  ...
  <?php else: ?>
    ...
    <div class="rpt-summary-card">
      <div class="val" style="color:#003087"><?= rpt_currency($total_debit) ?></div>
      <div class="lbl">Total Debits (In)</div>
    </div>
    <div class="rpt-summary-card">
      <div class="val" style="color:#c00"><?= rpt_currency($total_credit) ?></div>
      <div class="lbl">Total Credits (Out)</div>
    </div>
    <div class="rpt-summary-card">
      <div class="val"><?= rpt_currency($total_debit - $total_credit) ?></div>
        ...
        <?php else:
          foreach ($rows as $r): ?>
            <tr>
              ...
              <td style="font-size:11px;color:#666;max-width:200px" class="ns-text-truncate">
                <?= htmlspecialchars($r['memo'] ?? '-') ?></td>
              <td style="text-align:right;color:#003087;font-weight:700">
                <?= $r['entry_type'] === 'debit' ? rpt_currency($r['amount']) : '-' ?></td>
              <td style="text-align:right;color:#c00;font-weight:700">
                <?= $r['entry_type'] === 'credit' ? rpt_currency($r['amount']) : '-' ?></td>
            </tr>
