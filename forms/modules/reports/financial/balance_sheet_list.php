 ...
 */
function get_gl_bal($db, $id_or_subtype, $as_of, $is_id = true) {
    ...
    $row = $db->fetchOne("
        ...
        WHERE $field = ? AND j.entry_date <= ? AND a.is_deleted = 0
    ", [$id_or_subtype, $as_of]);
...
$bank_balance   = (float)($db->fetchOne("
    ...
    FROM journal_entries j
    JOIN accounts a ON j.account_id = a.id
    WHERE a.account_subtype = 'bank' AND a.id != 'acc-1010' AND j.entry_date <= ? AND a.is_deleted = 0
", [$as_of])['bal'] ?? 0);

$ar             = get_gl_bal($db, 'receivable', $as_of, false);
$inventory_val  = get_gl_bal($db, 'inventory', $as_of, false);

$total_current_assets = $cash_on_hand + $bank_balance + $ar + $inventory_val;
$total_assets         = $total_current_assets;

// ─── LIABILITIES ──────────────────────────────────────────────────────────────
$ap             = -get_gl_bal($db, 'payable', $as_of, false); // Liabilities are credits (negative in GL balance)
$tax_payable    = -get_gl_bal($db, 'tax', $as_of, false);

$total_liabilities = $ap + $tax_payable;

// ─── EQUITY ───────────────────────────────────────────────────────────────────
// Use Type-based balances for Income and Expenses
$revenue = -(float)($db->fetchOne("SELECT SUM(CASE WHEN j.entry_type = 'debit' THEN j.amount ELSE -j.amount END) AS v FROM journal_entries j JOIN accounts a ON j.account_id = a.id WHERE a.account_type = 'income' AND j.entry_date <= ? AND a.is_deleted = 0", [$as_of])['v'] ?? 0);

$expenses = (float)($db->fetchOne("SELECT SUM(CASE WHEN j.entry_type = 'debit' THEN j.amount ELSE -j.amount END) AS v FROM journal_entries j JOIN accounts a ON j.account_id = a.id WHERE a.account_type = 'expense' AND j.entry_date <= ? AND a.is_deleted = 0", [$as_of])['v'] ?? 0);
