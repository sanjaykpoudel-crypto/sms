...
$other_liability_accounts = $db->fetchAll("
    ...
    WHERE account_type = 'liability' 
      AND account_subtype NOT IN ('Accounts Payable', 'Other Current Liability')
      AND is_deleted = 0 AND is_active = 1
    ORDER BY account_name ASC
");
$other_liab_rows = [];
...
<style>
  ...
  .comp-table tr:hover:not(.grand-total-row):not(.total-row) {
    background: #f8fafc;
  }
  ...
  .comp-table .grand-total-row {
    font-weight: 800;
    background: #003087;
    color: #fff;
  }

  .comp-table .grand-total-row td {
    color: #fff !important;
  }
...
<div class="status-bar no-print">
  <div class="status-bar-box"
    style="<?= $is_balanced_this ? 'background:#d4edda;color:#1a7f37' : 'background:#f8d7da;color:#842029' ?>">
    <?= $this_fy ? htmlspecialchars($this_fy['name']) : 'Current Period' ?>:
    <?= $is_balanced_this ? 'Balanced ✓' : 'Out of Balance ✗' ?>
  </div>
  <?php if ($prev_fy): ?>
    <div class="status-bar-box"
      style="<?= $is_balanced_prev ? 'background:#d4edda;color:#1a7f37' : 'background:#f8d7da;color:#842029' ?>">
  ...
  <div class="ns-portlet-content">
    <table class="comp-table" id="tbl-comp-bs">
      ...
      <tbody>
        ...
        <tr>
          <td class="indent-1"><i class="fas fa-university" style="color:#003087;margin-right:6px"></i>Bank / Digital
            Balance</td>
        ...
        <tr>
          <td class="indent-1"><i class="fas fa-file-invoice-dollar" style="color:#1a7f37;margin-right:6px"></i>Accounts
            Receivable (AR)</td>
        ...
        <tr>
          <td class="indent-1"><i class="fas fa-boxes" style="color:#6f42c1;margin-right:6px"></i>Inventory (at Cost)
          </td>
        ...
        <?php if (!empty($other_assets_rows)): ?>
          ...
          <?php foreach ($other_assets_rows as $row): ?>
            <tr>
              <td class="indent-1"><i class="fas fa-building"
                  style="color:#475569;margin-right:6px"></i><?= htmlspecialchars($row['name']) ?></td>
        ...
        <tr>
          <td class="indent-1"><i class="fas fa-file-invoice-dollar" style="color:#c00;margin-right:6px"></i>Accounts
            Payable (AP)</td>
        ...
        <?php if (!empty($other_liab_rows)): ?>
          <tr>
            <td class="sub-sec-hdr" colspan="5">Other Liabilities</td>
          </tr>
          <?php foreach ($other_liab_rows as $row): ?>
            <tr>
              <td class="indent-1"><i class="fas fa-hand-holding-usd"
                  style="color:#475569;margin-right:6px"></i><?= htmlspecialchars($row['name']) ?></td>
              <td style="text-align:right"><?= rpt_currency($row['this']) ?></td>
              <td style="text-align:right"><?= rpt_currency($row['prev']) ?></td>
              <?= render_var_cols($row['this'], $row['prev']) ?>
        ...
        <tr>
          <td class="indent-1"><i class="fas fa-history" style="color:#2563eb;margin-right:6px"></i>Retained Earnings
            (Net Profit)</td>
        ...
        <?php foreach ($equity_rows as $row): ?>
          <tr>
            <td class="indent-1"><i class="fas fa-user-friends"
                style="color:#475569;margin-right:6px"></i><?= htmlspecialchars($row['name']) ?></td>
            <td style="text-align:right"><?= rpt_currency($row['this']) ?></td>
            <td style="text-align:right"><?= rpt_currency($row['prev']) ?></td>
            <?= render_var_cols($row['this'], $row['prev']) ?>
        ...
        <?php if (abs($reserve_this) > 0.005 || abs($reserve_prev) > 0.005): ?>
          <tr>
            <td class="indent-1"><i class="fas fa-calculator" style="color:#475569;margin-right:6px"></i>Inventory
              Adjustment Reserve</td>
