...
$expenses = $db->fetchAll("
    SELECT a.account_name, a.account_subtype,
           SUM(CASE WHEN j.entry_type = 'debit' THEN j.amount ELSE -j.amount END) as balance
    FROM accounts a
    JOIN journal_entries j ON a.id = j.account_id
    JOIN transaction_headers h ON j.header_id = h.id
...
$bs_balances = $db->fetchAll("
    SELECT a.account_name, a.account_type, a.normal_balance,
...
<head>
    ...
    <style>
        ...
        .data-table tr.total-row {
            ...
            border-bottom: 2px double #003087;
        }
...
<body>
    <div class="no-print" style="margin-bottom: 20px; text-align: right;">
        <button onclick="window.print()"
            style="padding: 6px 15px; background: #003087; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer;">Print
            Report</button>
        <button onclick="window.close()"
            style="padding: 6px 15px; background: #f3f4f6; color: #333; border: 1px solid #ccc; border-radius: 4px; margin-left: 8px; cursor: pointer;">Close
            Window</button>
    </div>
    ...
    <div class="print-header">
        ...
        <div style="font-size: 12px; color: #555; font-weight: 600; margin-top: 4px;">Fiscal Period:
            <?= htmlspecialchars($fy['name']) ?> (<?= $start ?> to <?= $end ?>)</div>
    </div>
    ...
    <table class="meta-table">
        <tr>
            ...
            <td style="font-weight: bold;">
                <?= htmlspecialchars($fy['closing_journal_no'] ?? 'JE-CLOSE-' . $fy['name']) ?></td>
        </tr>
        <tr>
    ...
    <div style="font-size: 11px; color:#666; margin-bottom: 8px; font-style: italic;">
        Note: The following balances represent the opening trial balance carried forward to the next fiscal year on date
        <?= date('Y-m-d', strtotime($end . ' +1 day')) ?>.
    </div>
    <table class="data-table">
        <thead>
            <tr>
    ...
    <div class="audit-grid">
        <div>
            ...
            <div><strong>Audit Log IP Address:</strong> <?= htmlspecialchars($audit['ip_address'] ?? '127.0.0.1') ?>
            </div>
            <div><strong>Audit Machine Name:</strong> <?= htmlspecialchars($audit['machine_name'] ?? 'Local Host') ?>
            </div>
