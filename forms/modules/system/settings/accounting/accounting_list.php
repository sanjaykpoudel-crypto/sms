...
<div class="ns-page-header">
    <h1 class="ns-page-title">
        Accounting Lists
        <a href="?page=system/settings/accounting/manage" class="ns-btn ns-btn-primary" id="new-entry-btn">
            ...
            <tbody>
                <?php foreach ($items as $i => $item): ?>
                    <tr>
                        <td><?php echo $i + 1; ?></td>
                        <td
                            data-search="<?php echo $item['type']; ?> <?php echo $type_labels[$item['type']] ?? $item['type']; ?>">
                            <span
                                style="font-size: 10px; font-weight: 700; color: #666; background: #eee; padding: 2px 6px; border-radius: 3px; text-transform: uppercase;">
                                <?php echo $type_labels[$item['type']] ?? $item['type']; ?>
                            </span>
                        </td>
                        <td><strong
                                style="color: var(--ns-primary);"><?php echo htmlspecialchars($item['name']); ?></strong>
                        </td>
                        <td>
                            <code
                                style="background: #f8f9fa; padding: 2px 4px; border-radius: 3px; border: 1px solid #ddd;">
                                <?php
                                if ($item['type'] == 'currency')
                                    echo htmlspecialchars($item['code'] . " (" . $item['symbol'] . ")");
                                else
                                    echo htmlspecialchars($item['code']);
                                ?>
                            </code>
                        </td>
                        <td>
                            <span style="font-weight: 600;">
                                ...
                                elseif ($item['type'] == 'currency')
                                    echo number_format($item['value'], 4);
                        ...
                        <td>
                            <span
                                style="color: <?php echo $item['is_active'] ? 'var(--ns-success)' : 'var(--ns-danger)'; ?>; font-weight: 600; font-size: 11px;">
                                <i
                                    class="fas <?php echo $item['is_active'] ? 'fa-check-circle' : 'fa-times-circle'; ?>"></i>
                                <?php echo $item['is_active'] ? 'Active' : 'Inactive'; ?>
                            </span>
                        </td>
                        <td>
                            <?php if (!empty($item['is_location'])): ?>
                                <a href="?page=system/settings/location/manage&id=<?php echo urlencode($item['id']); ?>"
                                    class="ns-btn" title="Edit Location">
                                    <i class="fas fa-edit"></i>
                                </a>
                            <?php else: ?>
                                <a href="?page=system/settings/accounting/manage&type=<?php echo $item['type']; ?>&id=<?php echo $item['id']; ?>"
    ...
    (function () {
        ...
        function initTypeFilter() {
            ...
            $select.on('change', function () {
                ...
                if (newBtn) {
                    if (val) {
                        newBtn.setAttribute('href', '?page=system/settings/accounting/manage&type=' + encodeURIComponent(val));
                    } else {
                        newBtn.setAttribute('href', '?page=system/settings/accounting/manage');
                    }
                }
