...
<style>
    ...
    .preview-summary-card .val {
        font-size: 15px;
        font-weight: 800;
        color: #003087;
    }

    .preview-summary-card .lbl {
        font-size: 10px;
        color: #64748b;
        text-transform: uppercase;
        font-weight: 600;
...
<div class="ns-page-header">
    <h1 class="ns-page-title">
        ...
        <button class="ns-btn ns-btn-primary" onclick="openFYModal()"><i class="fas fa-plus"></i> New Accounting
            Period</button>
    </h1>
</div>
...
<!-- TABS NAVIGATION -->
<div class="ns-tabs">
    <div class="ns-tab active" onclick="switchFyTab('tab-periods', this)"><i class="fas fa-calendar-alt"></i> Accounting
        Periods</div>
    <div class="ns-tab" onclick="switchFyTab('tab-preferences', this)"><i class="fas fa-sliders-h"></i> Closing
        Preferences & Rules</div>
</div>
    ...
    <div class="fy-dashboard">
        ...
        <div class="fy-list-panel">
            ...
            <div style="max-height: 600px; overflow-y: auto;">
                <?php if (empty($list)): ?>
                    <div style="padding: 30px; text-align: center; color: #888; font-size: 13px;">No accounting periods
                        defined.</div>
                <?php else:
                    foreach ($list as $row):
        ...
        <div class="fy-detail-panel">
            <div class="fy-header-bar">
                ...
                <?php if ($selected_fy): ?>
                    <button class="ns-btn" onclick="openFYModal(<?= htmlspecialchars(json_encode($selected_fy)) ?>)"
                        style="padding: 2px 10px; font-size: 11px;"><i class="fas fa-edit"></i> Edit</button>
                <?php endif; ?>
            </div>
            ...
            <div class="fy-detail-body">
                <?php if (!$selected_fy): ?>
                    <div
                        ...
                        <div style="font-size: 12px; margin-top: 6px;">Select an accounting period from the list on the left
                            to manage it.</div>
                    </div>
                <?php else:
                    ...
                    <div class="fy-meta-grid">
                        <div class="fy-meta-item">
                            ...
                            <div class="val" style="color:#003087; font-size:15px;">
                                <?= htmlspecialchars($selected_fy['name']) ?></div>
                        </div>
                        <div class="fy-meta-item">
                        ...
                        <div class="fy-meta-item">
                            ...
                            <div class="val">
                                <span class="badge"
                                    style="background: <?= $is_closed ? '#f3f4f6' : '#e6fffa'; ?>; color: <?= $is_closed ? '#4b5563' : '#047481'; ?>; padding: 2px 8px; border-radius: 4px; border: 1px solid currentColor; font-size: 11px; text-transform: uppercase; font-weight: 800;">
                                    <?= $selected_fy['status'] ?>
                                </span>
                            </div>
                        </div>
                        <?php if ($is_closed): ?>
                            <div class="fy-meta-item">
                                ...
                                <div class="val">
                                    <?= htmlspecialchars($selected_fy['closed_by_name'] ?? $selected_fy['closed_by'] ?? 'System') ?>
                                </div>
                            </div>
                            <div class="fy-meta-item">
                                <div class="lbl">Closed Date / Time</div>
                                <div class="val"><?= $selected_fy['closing_date'] ?>
                                    <?= $selected_fy['closed_timestamp'] ? date('H:i', strtotime($selected_fy['closed_timestamp'])) : '' ?>
                                </div>
                            </div>
                        <?php elseif ($selected_fy['status'] === 'reopened'): ?>
                            <div class="fy-meta-item">
                                ...
                                <div class="val">
                                    <?= htmlspecialchars($selected_fy['reopened_by_name'] ?? $selected_fy['reopened_by'] ?? 'User') ?>
                                </div>
                            </div>
                            <div class="fy-meta-item">
                        ...
                        <?php if (!empty($selected_fy['notes'])): ?>
                            <div class="fy-meta-item" style="grid-column: 1 / -1;">
                                ...
                                <div class="val" style="font-weight: 500; font-style: italic; color: #475569;">
                                    <?= nl2br(htmlspecialchars($selected_fy['notes'])) ?></div>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="fy-results-container" id="results-box">
                        <div class="results-header">
                            ...
                            <button class="ns-btn" style="padding: 2px 8px; font-size: 11px;" onclick="closeResultsBox()"><i
...
<div id="tab-preferences" class="ns-fy-tab-content" style="display:none;">
    <div
        style="background:#fff; border:1px solid #dde2e8; border-radius:8px; padding:24px; box-shadow:0 1px 4px rgba(0,0,0,.05);">
        <form id="fy-prefs-form" onsubmit="return handleFyPrefsSave(event)">
            <div
                style="font-size:15px; font-weight:800; color:#003087; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:20px; border-bottom:1px solid #eef0f2; padding-bottom:10px;">
            ...
            <div class="ns-form-row">
                <div class="ns-form-group">
                    ...
                    <select name="fy_retained_earnings_account" class="ns-select" required>
                        ...
                        <?php foreach ($accounts as $acc): ?>
                            <option value="<?php echo $acc['id']; ?>" <?php echo ($settings['fy_retained_earnings_account'] ?? 'acc-3200') == $acc['id'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($acc['account_name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="ns-form-group">
                    ...
                    <select name="fy_income_summary_account" class="ns-select">
                        <option value="">-- Select Account --</option>
                        <?php foreach ($accounts as $acc): ?>
                            <option value="<?php echo $acc['id']; ?>" <?php echo ($settings['fy_income_summary_account'] ?? '') == $acc['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($acc['account_name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="ns-form-group">
                    ...
                    <select name="fy_dividend_payable_account" class="ns-select">
                        <option value="">-- Select Account --</option>
                        <?php foreach ($accounts as $acc): ?>
                            <option value="<?php echo $acc['id']; ?>" <?php echo ($settings['fy_dividend_payable_account'] ?? '') == $acc['id'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($acc['account_name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="ns-form-row" style="margin-top: 15px;">
                ...
                <div class="ns-form-group">
                    ...
                    <input type="text" name="fy_closing_prefix" class="ns-input"
                        value="<?php echo htmlspecialchars($settings['fy_closing_prefix'] ?? 'JE-CLOSE-'); ?>"
                        placeholder="e.g. JE-CLOSE-">
                </div>
                <div class="ns-form-group">
            ...
            <div
                style="margin-top: 25px; padding-top: 15px; border-top: 1px solid #eee; display: flex; justify-content: flex-end;">
                <button type="submit" class="ns-btn ns-btn-primary" id="btn-save-fy-prefs"><i class="fas fa-save"></i>
                    Save Closing Preferences</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="ns-modal-content">
        ...
        <form id="fy-form" onsubmit="saveFY(event)">
            ...
            <div class="ns-form-group">
                ...
                <textarea name="notes" id="form-notes" class="ns-input" style="height: 80px; resize: vertical;"
                    placeholder="Add initial notes or description..."></textarea>
            </div>
    ...
    <div class="ns-modal-content">
        ...
        <form id="close-form" onsubmit="executeClose(event)">
            <div style="font-size: 13px; color: #555; margin-bottom: 15px; line-height: 1.5;">
                ...
                Closing this fiscal year will lock all transactions in this period and generate closing journal entries
                to Retained Earnings and opening balances for the next year.
            </div>
            <div class="ns-form-group">
                <label class="ns-label">Closing Notes / Comments</label>
                <textarea name="notes" id="close-notes" class="ns-input" style="height: 100px; resize: vertical;"
                    placeholder="Enter details about this year-end closing..." required></textarea>
            </div>
            <div class="ns-modal-footer">
                <button type="button" class="ns-btn" onclick="closeCloseModal()">Cancel</button>
                <button type="submit" class="ns-btn ns-btn-primary" id="btn-execute-close"><i class="fas fa-lock"></i>
                    Finalize Period Close</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="ns-modal-content">
        ...
        <form id="reopen-form" onsubmit="executeReopen(event)">
            <div style="font-size: 13px; color: #c00; margin-bottom: 15px; line-height: 1.5; font-weight: 600;">
                ...
                Are you sure you want to reopen this fiscal year? Reopening will delete the system-generated closing
                journal and next year's opening balances!
            </div>
            <div class="ns-form-group">
                <label class="ns-label">Reason for Reopening *</label>
                <textarea name="reason" id="reopen-reason" class="ns-input" style="height: 100px; resize: vertical;"
                    placeholder="Reason is required for audit trail..." required></textarea>
            </div>
            <div class="ns-modal-footer">
                <button type="button" class="ns-btn" onclick="closeReopenModal()">Cancel</button>
                <button type="submit" class="ns-btn" id="btn-execute-reopen" style="background:#d32f2f; color:#fff;"><i
                        class="fas fa-unlock"></i> Reopen Period</button>
            </div>
        </form>
    </div>
</div>
