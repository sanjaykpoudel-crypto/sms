...
echo "====================================================================\n";
echo " COMPLETE GL RE-POSTING & RE-CONCILIATION ENGINE\n";
echo " Re-generating double-entry GL journals for all historical headers\n";
echo "====================================================================\n\n";

// Purge incomplete journal entries (headers with 0 lines)
$pdo->exec("
    DELETE FROM journal_entries 
    WHERE je_id NOT IN (SELECT DISTINCT je_id FROM journal_lines)
");
...
foreach ($headers as $h) {
    ...
    // Check if JE already has valid lines
    $line_cnt = (int)$pdo->query("
        SELECT COUNT(*) FROM journal_lines jl
        JOIN journal_entries je ON jl.je_id = je.je_id
        WHERE je.transaction_id = " . $pdo->quote($id)
    )->fetchColumn();
...
echo "\nGL Re-posting Complete:\n";
echo "  - Posted:  {$posted_count}\n";
echo "  - Skipped (Already posted with lines): {$skipped_count}\n";
